...
add_filter( 'site_transient_update_themes', 'ks_update_themes' );

function ks_update_themes( $transient ) {
...
add_filter( 'upgrader_package_options', 'ks_upgrader_package_options', 10, 1 );

/**
 * @param $options
 * @return mixed
 */
function ks_upgrader_package_options( $options ) {
    ...
    if ( isset( $options['destination'] ) && isset( $options['type'] ) && 'theme' === $options['type'] ) {
        ...
        $options['destination'] = get_theme_root() . '/' . $stylesheet;
    }
    return $options;
}