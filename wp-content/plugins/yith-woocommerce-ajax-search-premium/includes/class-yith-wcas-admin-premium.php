...
if ( ! class_exists( 'YITH_WCAS_Admin_Premium' ) ) {
	...
	class YITH_WCAS_Admin_Premium extends YITH_WCAS_Admin {
		...
		public function __construct() {
			...
			add_filter( 'yith_wcas_search_fields_saved_option', array(
				$this,
		...
		public function add_store_tools_tab( $args ) {
			...
			$args['your_store_tools'] = array(
				'items' => array(
					...
					'customize-my-account'   => array(
						...
						'description'    => _x( 'Customize the My Account page of your customers by creating custom sections with promotions and ad-hoc content based on your needs.', '[YOUR STORE TOOLS TAB] Description for plugin YITH WooCommerce Customize My Account', 'yith-woocommerce-ajax-search' ),
			...
			$args['welcome_modals'] = array(
				'on_close'    => function () { update_option( 'ywcas-plugin-welcome-modal', 'no' ); },
				'modals'  => array(
					'welcome' => array(
						'type'        => 'welcome',
						...
						'items'       => array(
							...
							'add-shortcode'        => array(
								...
								'url'         => add_query_arg( array( 'page' => 'yith_wcas_panel', 'tab'=>'shortcodes' ), admin_url( 'admin.php' ) ),
							),
		...
		public function add_premium_option_in_general_tab( $options ) {
			$results_section_options = array(
				...
				'include_variations'                  => array(
					...
					'yith-type' => 'onoff',
					'default'   => 'no',
					'class'     => ywcas_get_disabled_class(),
					'deps'      => array(
						'id'    => 'yith_wcas_enable_autocomplete',
						'value' => 'yes',
					),
				),
				'disable_include_variations_message'  => array(
					'type'      => 'yith-field',
					'yith-type' => 'html',
			...
			$offset = array_search( 'section_end_search_settings', array_keys( $options['general'] ) );
