...
use Barn2\Plugin\WC_Filters\Dependencies\Illuminate\Support\Enumerable;
use Barn2\Plugin\WC_Filters\Dependencies\Illuminate\Support\HigherOrderCollectionProxy;
use Barn2\Plugin\WC_Filters\Dependencies\Illuminate\Support\HigherOrderWhenProxy;
use JsonSerializable;
use Barn2\Plugin\WC_Filters\Dependencies\Symfony\Component\VarDumper\VarDumper;
use Traversable;
...
trait EnumeratesValues
{
    ...
    public function containsStrict($key, $value = null)
    {
        if (\func_num_args() === 2) {
            return $this->contains(function ($item) use($key, $value) {
                return \Barn2\Plugin\WC_Filters\Helpers::data_get($item, $key) === $value;
            });
        }
    ...
    public function whereIn($key, $values, $strict = \false)
    {
        ...
        return $this->filter(function ($item) use($key, $values, $strict) {
            return \in_array(\Barn2\Plugin\WC_Filters\Helpers::data_get($item, $key), $values, $strict);
        });
    }
    /**
     * Filter items by the given key value pair using strict comparison.
    ...
    public function whereNotBetween($key, $values)
    {
        return $this->filter(function ($item) use($key, $values) {
            return \Barn2\Plugin\WC_Filters\Helpers::data_get($item, $key) < \reset($values) || \Barn2\Plugin\WC_Filters\Helpers::data_get($item, $key) > \end($values);
        });
    }
    /**
     * Filter items by the given key value pair.
    ...
    public function reduceSpread(callable $callback, ...$initial)
    {
        ...
        foreach ($this as $key => $value) {
            ...
            if (!\is_array($result)) {
                throw new UnexpectedValueException(\sprintf("%s::reduceMany expects reducer to return an array, but got a '%s' instead.", \Barn2\Plugin\WC_Filters\Helpers::class_basename(static::class), \gettype($result)));
            }
        }
        return $result;
    }
    ...
    protected function operatorForWhere($key, $operator = null, $value = null)
    {
        ...
        return function ($item) use($key, $operator, $value) {
            $retrieved = \Barn2\Plugin\WC_Filters\Helpers::data_get($item, $key);
    ...
    protected function valueRetriever($value)
    {
        ...
        return function ($item) use($value) {
            return \Barn2\Plugin\WC_Filters\Helpers::data_get($item, $value);
        };
    }
    /**
