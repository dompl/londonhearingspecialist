...
namespace Barn2\Plugin\WC_Filters\Dependencies\Illuminate\Database\Query;

use BackedEnum;
use Closure;
use DateTimeInterface;
use Barn2\Plugin\WC_Filters\Dependencies\Illuminate\Contracts\Support\Arrayable;
use Barn2\Plugin\WC_Filters\Dependencies\Illuminate\Database\Concerns\BuildsQueries;
use Barn2\Plugin\WC_Filters\Dependencies\Illuminate\Database\Concerns\ExplainsQueries;
use Barn2\Plugin\WC_Filters\Dependencies\Illuminate\Database\ConnectionInterface;
use Barn2\Plugin\WC_Filters\Dependencies\Illuminate\Database\Eloquent\Builder as EloquentBuilder;
use Barn2\Plugin\WC_Filters\Dependencies\Illuminate\Database\Eloquent\Relations\Relation;
use Barn2\Plugin\WC_Filters\Dependencies\Illuminate\Database\Query\Grammars\Grammar;
use Barn2\Plugin\WC_Filters\Dependencies\Illuminate\Database\Query\Processors\Processor;
use Barn2\Plugin\WC_Filters\Dependencies\Illuminate\Pagination\Paginator;
...
use Barn2\Plugin\WC_Filters\Dependencies\Illuminate\Support\LazyCollection;
use Barn2\Plugin\WC_Filters\Dependencies\Illuminate\Support\Str;
use Barn2\Plugin\WC_Filters\Dependencies\Illuminate\Support\Traits\ForwardsCalls;
use Barn2\Plugin\WC_Filters\Dependencies\Illuminate\Support\Traits\Macroable;
...
class Builder
{
    ...
    {
        return !\is_string($operator) || !\in_array(\strtolower($operator), $this->operators, \true) && !\in_array(\strtolower($operator), $this->grammar->getOperators(), \true);
    }
    /**
    ...
    {
        return \in_array(\strtolower($operator), $this->bitwiseOperators, \true) || \in_array(\strtolower($operator), $this->grammar->getBitwiseOperators(), \true);
    }
    /**
    ...
    {
        return $this->whereIn($column, $values, $boolean, \true);
    }
    /**
    ...
    {
        return $this->whereIntegerInRaw($column, $values, $boolean, \true);
    }
    /**
    ...
    {
        return $this->whereNull($columns, $boolean, \true);
    }
    /**
    ...
    {
        return $this->whereBetween($column, $values, $boolean, \true);
    }
    /**
    ...
    {
        return $this->whereBetweenColumns($column, $values, $boolean, \true);
    }
    /**
    ...
    {
        return $this->whereExists($callback, $boolean, \true);
    }
    /**
     * Add a where not exists clause to the query.
     *
    ...
    {
        return $this->orWhereExists($callback, \true);
    }
    /**
     * Add an exists clause to the query.
     *
    ...
    {
        return $this->whereJsonContains($column, $value, $boolean, \true);
    }
    /**
     ...
     * @return $this
     */
    public function lock($value = \true)
    {
        $this->lock = $value;
    ...
    {
        return \Barn2\Plugin\WC_Filters\Helpers::collect($this->onceWithColumns(Arr::wrap($columns), function () {
    ...
    public function paginate($perPage = 15, $columns = ['*'], $pageName = 'page', $page = null)
    {
        ...
        $results = $total ? $this->forPage($page, $perPage)->get($columns) : \Barn2\Plugin\WC_Filters\Helpers::collect();
    ...
    public function simplePaginate($perPage = 15, $columns = ['*'], $pageName = 'page', $page = null)
    {
        ...
        return $this->simplePaginator($this->get($columns), $perPage, $page, ['path' => Paginator::resolveCurrentPath(), 'pageName' => $pageName]);
    }
    /**
     * Get a paginator only supporting simple next and previous links.
     *
    ...
    {
        $this->enforceOrderBy();
        return \Barn2\Plugin\WC_Filters\Helpers::collect($this->orders ?? $this->unionOrders ?? [])->filter(function ($order) {
    ...
    public function cursor()
    {
        ...
        return new LazyCollection(function () {
            yield from $this->connection->cursor($this->toSql(), $this->getBindings(), !$this->useWritePdo);
        });
    }
    /**
    ...
    public function pluck($column, $key = null)
    {
        ...
        if (empty($queryResult)) {
            return \Barn2\Plugin\WC_Filters\Helpers::collect();
        }
    ...
    protected function stripTableForPluck($column)
    {
        ...
        return \Barn2\Plugin\WC_Filters\Helpers::last(\preg_split('~' . $separator . '~i', $column));
    }
    /**
    ...
    protected function pluckFromObjectColumn($queryResult, $column, $key)
    {
        ...
        return \Barn2\Plugin\WC_Filters\Helpers::collect($results);
    }
    /**
    ...
    {
        return $this->exists() ? \true : $callback();
    }
    /**
    ...
    {
        return $this->doesntExist() ? \true : $callback();
    }
    /**
    ...
    {
        $results = $this->cloneWithout($this->unions || $this->havings ? [] : ['columns'])->cloneWithoutBindings($this->unions || $this->havings ? [] : ['select'])->setAggregate($function, $columns)->get($columns);
        if (!$results->isEmpty()) {
    ...
    public function insertOrIgnore(array $values)
    {
        ...
        $this->applyBeforeQueryCallbacks();
        return $this->connection->affectingStatement($this->grammar->compileInsertOrIgnore($this, $values), $this->cleanBindings(Arr::flatten($values, 1)));
    }
    /**
    ...
    public function updateFrom(array $values)
    {
        ...
        return $this->connection->update($sql, $this->cleanBindings($this->grammar->prepareBindingsForUpdateFrom($this->bindings, $values)));
    }
    /**
    ...
    public function upsert(array $values, $uniqueBy, $update = null)
    {
        ...
        $this->applyBeforeQueryCallbacks();
        $bindings = $this->cleanBindings(\array_merge(Arr::flatten($values, 1), \Barn2\Plugin\WC_Filters\Helpers::collect($update)->reject(function ($value, $key) {
            return \is_int($key);
        })->all()));
    ...
    public function addBinding($value, $type = 'where')
    {
        ...
        if (\is_array($value)) {
            $this->bindings[$type] = \array_values(\array_map([$this, 'castBinding'], \array_merge($this->bindings[$type], $value)));
        } else {
    ...
    {
        return \Barn2\Plugin\WC_Filters\Helpers::collect($bindings)->reject(function ($binding) {
    ...
    {
        return \is_array($value) ? \Barn2\Plugin\WC_Filters\Helpers::head(Arr::flatten($value)) : $value;
    }
    /**
    ...
    {
        return \Barn2\Plugin\WC_Filters\Helpers::tap($this->clone(), function ($clone) use($properties) {
    ...
    {
        return \Barn2\Plugin\WC_Filters\Helpers::tap($this->clone(), function ($clone) use($except) {
