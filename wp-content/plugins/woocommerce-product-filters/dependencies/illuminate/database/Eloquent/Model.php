...
use ArrayAccess;
use Barn2\Plugin\WC_Filters\Dependencies\Illuminate\Contracts\Broadcasting\HasBroadcastChannel;
use Barn2\Plugin\WC_Filters\Dependencies\Illuminate\Contracts\Queue\QueueableCollection;
use Barn2\Plugin\WC_Filters\Dependencies\Illuminate\Contracts\Queue\QueueableEntity;
use Barn2\Plugin\WC_Filters\Dependencies\Illuminate\Contracts\Routing\UrlRoutable;
use Barn2\Plugin\WC_Filters\Dependencies\Illuminate\Contracts\Support\Arrayable;
use Barn2\Plugin\WC_Filters\Dependencies\Illuminate\Contracts\Support\CanBeEscapedWhenCastToString;
use Barn2\Plugin\WC_Filters\Dependencies\Illuminate\Contracts\Support\Jsonable;
use Barn2\Plugin\WC_Filters\Dependencies\Illuminate\Database\ConnectionResolverInterface as Resolver;
use Barn2\Plugin\WC_Filters\Dependencies\Illuminate\Database\Eloquent\Collection as EloquentCollection;
use Barn2\Plugin\WC_Filters\Dependencies\Illuminate\Database\Eloquent\Relations\BelongsToMany;
use Barn2\Plugin\WC_Filters\Dependencies\Illuminate\Database\Eloquent\Relations\Concerns\AsPivot;
use Barn2\Plugin\WC_Filters\Dependencies\Illuminate\Database\Eloquent\Relations\HasManyThrough;
use Barn2\Plugin\WC_Filters\Dependencies\Illuminate\Database\Eloquent\Relations\Pivot;
use Barn2\Plugin\WC_Filters\Dependencies\Illuminate\Support\Arr;
use Barn2\Plugin\WC_Filters\Dependencies\Illuminate\Support\Collection as BaseCollection;
...
{
    use Concerns\HasAttributes, Concerns\HasEvents, Concerns\HasGlobalScopes, Concerns\HasRelationships, Concerns\HasTimestamps, Concerns\HidesAttributes, Concerns\GuardsAttributes, ForwardsCalls;
    /**
     ...
     * @var bool
     */
    protected static $isBroadcasting = \true;
    /**
    ...
    protected static function bootTraits()
    {
        ...
        foreach (\Barn2\Plugin\WC_Filters\Helpers::class_uses_recursive($class) as $trait) {
            $method = 'boot' . \Barn2\Plugin\WC_Filters\Helpers::class_basename($trait);
            if (\method_exists($class, $method) && !\in_array($method, $booted)) {
                \forward_static_call([$class, $method]);
                $booted[] = $method;
            }
            if (\method_exists($class, $method = 'initialize' . \Barn2\Plugin\WC_Filters\Helpers::class_basename($trait))) {
     ...
     * @return void
     */
    public static function preventLazyLoading($value = \true)
    {
    ...
    {
        return \Barn2\Plugin\WC_Filters\Helpers::collect($columns)->map(function ($column) {
    ...
    {
        $model = $this->newInstance([], \true);
        $model->setRawAttributes((array) $attributes, \true);
    ...
    protected function incrementOrDecrement($column, $amount, $extra, $method)
    {
        ...
        return \Barn2\Plugin\WC_Filters\Helpers::tap($this->setKeysForSaveQuery($query)->{$method}($column, $amount, $extra), function () use($column) {
            $this->syncChanges();
    ...
    public function save(array $options = [])
    {
        ...
        if ($this->exists) {
            $saved = $this->isDirty() ? $this->performUpdate($query) : \true;
        } else {
    ...
    protected function finishSave(array $options)
    {
        ...
        if ($this->isDirty() && ($options['touch'] ?? \true)) {
            $this->touchOwners();
        }
        $this->syncOriginal();
    }
    ...
    public function refresh()
    {
        ...
        $this->load(\Barn2\Plugin\WC_Filters\Helpers::collect($this->relations)->reject(function ($relation) {
            return $relation instanceof Pivot || \is_object($relation) && \in_array(AsPivot::class, \Barn2\Plugin\WC_Filters\Helpers::class_uses_recursive($relation), \true);
        })->keys()->all());
        $this->syncOriginal();
        return $this;
    }
    ...
    public function replicate(array $except = null)
    {
        ...
        return \Barn2\Plugin\WC_Filters\Helpers::tap(new static(), function ($instance) use($attributes) {
    ...
    {
        return $this->table ?? Str::snake(Str::pluralStudly(\Barn2\Plugin\WC_Filters\Helpers::class_basename($this)));
    }
    /**
    ...
    {
        return Str::snake(\Barn2\Plugin\WC_Filters\Helpers::class_basename($this)) . '_' . $this->getKeyName();
    }
    /**
    ...
    {
        return \str_replace('\\', '.', \get_class($this)) . '.{' . Str::camel(\Barn2\Plugin\WC_Filters\Helpers::class_basename($this)) . '}';
    }
    /**
