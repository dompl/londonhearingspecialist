...
use Barn2\Plugin\WC_Filters\Dependencies\Setup_Wizard\Interfaces\Deferrable;
...
class Api implements JsonSerializable
{
    ...
    public function save_fields($request)
    {
        ...
        if (empty($step) || !$step instanceof Step) {
            return self::send_error_response(['message' => __('Could not find the appropriate step.', 'woocommerce-product-filters')]);
        }
    ...
    public function handle_license($request)
    {
        ...
        if (empty($license_key)) {
            return self::send_error_response(['message' => __('Please enter a license key.', 'woocommerce-product-filters')]);
        }
        if (!\in_array($action, $allowed_actions, \true)) {
            return self::send_error_response(['message' => __('Invalid action requested.', 'woocommerce-product-filters')]);
        }
        switch ($action) {
            case 'activate':
