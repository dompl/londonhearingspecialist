...
use Barn2\Plugin\WC_Filters\Dependencies\Lib\Admin\Notices;
...
use Barn2\Plugin\WC_Filters\Integrations\Plugin_Product_Bundles;
use Barn2\Plugin\WC_Filters\Integrations\Theme_Betheme;
use Barn2\Plugin\WC_Filters\Integrations\Theme_Bricks;
use Barn2\Plugin\WC_Filters\Integrations\Theme_Shoptimizer;
...
class Plugin extends Premium_Plugin implements Licensed_Plugin, Registerable, Translatable, Service_Provider {
	...
	public function __construct( $file = null, $version = '1.0.0' ) {
		...
		$this->add_service( 'db', $database_capsule, true );
		$this->add_service( 'plugin_setup', new Plugin_Setup( $this->get_file(), $this ), true );
		$this->add_service( 'widget', new Widget( $this ), true );
	}
	...
	public function register() {
		...
		add_action( 'plugins_loaded', [ $this, 'maybe_load_plugin' ], 1 );
	...
	public function add_services() {
		if ( is_admin() ) {
			$this->add_service( 'admin', new Admin_Controller( $this, $this->get_license_setting(), $this->get_license() ) );
		}

		$this->add_service( 'setup_wizard', new Setup_Wizard( $this ) );

		if ( class_exists( 'WC_Action_Queue' ) ) {
			$this->add_service( 'queue', new \WC_Action_Queue() );
		}
		...
		if ( $this->has_valid_license() ) {
			...
			$services['css_vars']  = new CSS_Variables();

			$services['integration_theme/avada']                   = new Theme_Avada();
			$services['integration_theme/betheme']                 = new Theme_Betheme();
			...
			$services['integration_theme/bricks']                  = new Theme_Bricks();
			$services['integration_theme/shoptimizer']             = new Theme_Shoptimizer();
			...
			$services['integration_plugin/product-bundles']        = new Plugin_Product_Bundles();
		}

		$services['meta_fields'] = new Meta_Fields( $services );

		foreach ( $services as $id => $service ) {
			$this->add_service( $id, $service );
		}
	}
