...
class Plugin_Product_Table implements Registerable, JsonSerializable {
	...
	public function init() {
		...
		add_filter( 'wcf_fallback_attach_search_query_to_filter', [ $this, 'adjust_ranged_parameters' ], 10, 3 );
	...
	private function should_enqueue() {
		 ...
		 * @return boolean
		 */
		$enabled = apply_filters( 'wcf_product_table_integration_assets_enabled', $enabled );
			...
			function ( $terms, $filter ) use ( $table, $categories, $restricted_to ) {
				if ( $filter instanceof Taxonomy ) {
					foreach ( $terms as $index => $term ) {
						...
						} elseif ( isset( $term['slug'] ) && in_array( $term['slug'], $restricted_to, true ) ) {
								unset( $terms[ $index ] );
						}
					}
				}
	...
	public function table_output( $result, $output, $table ) {
		...
		if ( $output === 'html' ) {
			...
			ob_start();
			if ( $this->should_display_additional_elements() ) {
	 ...
	 * @return boolean
	 */
	private function should_display_additional_elements() {
		$should_display_additional_elements = true;

		if ( is_shop() ) {
			$should_display_additional_elements = false;
		}

		/**
		 * Determine if the additional elements should be displayed.
		 *
		 * @param boolean $should_display_additional_elements default is true.
		 * @return boolean true if the additional elements should be displayed.
		 */
		$should_display_additional_elements = apply_filters( 'wcf_product_table_integration_display_additional_elements', $should_display_additional_elements );
	 ...
	 *
	 * @param mixed $value
	 * @param Attribute|Taxonomy $filter
	 * @param Collection $parameters
	 * @return mixed
	 */
	public function adjust_ranged_parameters( $value, $filter, $parameters ) {
		if ( $filter instanceof Taxonomy || $filter instanceof Attribute ) {
			if ( is_array( $value ) ) {
				$value = implode( ',', $value );
			}
		}
