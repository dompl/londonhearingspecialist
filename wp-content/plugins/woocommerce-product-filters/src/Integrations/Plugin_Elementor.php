...
class Plugin_Elementor implements Registerable, JsonSerializable {
	...
	protected function hook_into_widgets() {
		...
		// Manually add the controls to the shortcode widget.
		add_action( 'elementor/element/shortcode/section_shortcode/after_section_end', [ $this, 'register_shortcode_controls' ], 10, 2 );
	}
	 ...
	 *
	 * This is needed for those cases where multiple integrations are used
	 * and the automated detection fails.
	 *
	 * @param object $element Elementor widget instance.
	 * @param array $args
	 * @return void
	 */
	public function register_shortcode_controls( $element, $args ) {
		$element->start_controls_section(
			'wcf_section',
			[
				'label' => __( 'WooCommerce Product Filters', 'woocommerce-product-filters' ),
		...
		$element->add_control(
			'enable_wcf',
			[
				'label'        => __( 'Enable filtering elements', 'woocommerce-product-filters' ),
				'description'  => __( 'This will enable the filtering elements for this shortcode.', 'woocommerce-product-filters' ),
				'type'         => \Elementor\Controls_Manager::SWITCHER,
				'label_on'     => __( 'Yes', 'woocommerce-product-filters' ),
				'label_off'    => __( 'No', 'woocommerce-product-filters' ),
				'return_value' => 'yes',
	...
	public function add_closing_wrapper( $content, $widget ) {
		...
		if ( isset( $this->supported_widgets[ $name ] ) && 'yes' === $widget->get_settings( 'enable_wcf' ) ) {
			...
			if ( Filters::is_taxonomy_page() ) {
				global $wp_query;
				$products_ids = $this->get_all_post_ids_from_query( $wp_query );
			} else {
		...
		// This is for any other element that might need future support.
		$extra_allowed = [ 'shortcode' ];

		if ( in_array( $name, $extra_allowed, true ) && 'yes' === $widget->get_settings( 'enable_wcf' ) ) {
			$display_service = wcf()->get_service( 'display' );
			...
			return $mobile . '<div class="wcf-elementor-extra-wrapper">' . $content . '</div>';
		}
	...
	public function catch_elementor_page() {
		...
		add_action(
			...
			function () {
				...
				$this->maybe_force_enqueue_of_wpt_asset( $widgets );
	 ...
	 * @return boolean
	 */
	private function maybe_force_enqueue_of_wpt_asset( array $widgets ) {
		$enqueue = false;

		if ( in_array( 'shortcode', $widgets, true ) && function_exists( '\Barn2\Plugin\WC_Product_Table\wpt' ) ) {
			$enqueue = true;
		}
		 ...
		 *
		 * @param boolean $enqueue Whether to force enqueue or not.
		 * @param array $widgets List of widgets used on the page.
		 * @return boolean
		 */
		return apply_filters( 'wcf_elementor_force_enqueue_of_wpt_asset', $enqueue, $widgets );
	}
	...
	public function inject_template() {
		...
		if ( empty( $this->cache->get_post_ids() ) && $this->cache->get_filters()->isEmpty() ) {
	 ...
	 * @return array
	 */
	private function get_all_post_ids_from_query( $wp_query ) {
		/**
		 * Filter: determine if all posts should be loaded
		 * when viewing a taxonomy archive page.
		 *
		 * @param boolean $should_load_all_posts
		 * @param \WP_Query $wp_query
		 * @return boolean
		 */
		$should_load_all_posts = apply_filters( 'wcf_elementor_tax_query_load_all_posts', true, $wp_query );

		if ( ! $should_load_all_posts ) {
			return $wp_query->posts;
		}

		$query_vars = $wp_query->query_vars;

		// Now run a query to fetch all the post ids in one go.
		$query_vars['fields']         = 'ids';
		$query_vars['posts_per_page'] = 99999;
