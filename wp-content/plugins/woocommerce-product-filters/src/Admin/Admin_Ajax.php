...
class Admin_Ajax implements Registerable {
	...
	public function duplicate_group(): void {
		...
		$this->fix_filter_ids( $new_group );
		$this->fix_parent_filter_ids( $new_group );
	...
	/**
	 * Navigate through the filters and fix the "filter_id" property
	 * inside the options array but only if "filter_id" exists.
	 *
	 * The new filter_id is the one of the newly created filter.
	 *
	 * This is needed because when duplicating a group, the filters
	 * are duplicated too but the filter_id is not updated.
	 *
	 * @param Group $group
	 * @return void
	 */
	private function fix_filter_ids( Group $group ): void {
		$filters = $group->get_filters();
		...
		if ( ! empty( $filters ) ) {
			foreach ( $filters as $filter ) {
				...
				if ( isset( $options['filter_id'] ) ) {
					$options['filter_id'] = $filter->getID();
	...
	/**
	 * Navigate through the filters and fix the "parent_filter" column
	 * but only if the "parent_filter" is different than 0.
	 *
	 * To retrieve the new parent_filter we need to query the database
	 * and get the new filter_id of the filter in the group that has
	 * the option "attributes_mode" set to "all".
	 *
	 * @param Group $group
	 * @return void
	 */
	private function fix_parent_filter_ids( Group $group ): void {
		// Get all the filters.
		$filters = $group->get_filters();

		// Find the filter that has the "attributes_mode" option set to "all".
		$all_atts_filter = $filters->where( 'filter_by', 'attributes' )
									->where( 'options.attributes_mode', 'all' )
									->first();

		// If we found the filter, get the new filter_id.
		if ( $all_atts_filter instanceof Filter ) {
			$new_filter_id = $all_atts_filter->getID();

			// Update the parent_filter column for all the filters where the parent_filter is different than 0.
			$filters->where( 'parent_filter', '!=', 0 )
					->each(
						function ( Filter $filter ) use ( $new_filter_id ) {
							$filter->update(
								[
									'parent_filter' => $new_filter_id,
								]
							);
						}
					);
	...
	private function create_or_update_filter( array $filter, $group_id, $priority ) {
		...
		$slug        = sanitize_title_with_dashes( $filter_name, '', 'save' );
		$options     = ( new Collection( wc_clean( $filter ) ) );
		...
		$options     = ( $this->maybe_attach_taxonomy_setting( $options ) )->except( [ 'filter_name', 'filter_by' ] );
		$filter_by   = $this->maybe_adjust_filter_by( $options, $filter_by );
	 ...
	 *
	 * @param Collection $options The filter options.
	 * @return Collection The filter options.
	 */
	private function maybe_attach_taxonomy_setting( Collection $options ) {
		$filter_by = $options->get( 'filter_by' ) ?? false;

		// If filter_by starts with "tax_" then it's a taxonomy.
		if ( $filter_by && strpos( $filter_by, 'tax_' ) === 0 ) {
			$taxonomy = str_replace( 'tax_', '', $filter_by );

			$options->put( 'taxonomy', $taxonomy );
		}
	 ...
	 *
	 * @param Collection $options The filter options.
	 * @param string $filter_by The filter_by option.
	 * @return string The filter_by option.
	 */
	private function maybe_adjust_filter_by( Collection $options, $filter_by ) {
		$filter_mode = $options->get( 'filter_method' ) ?? 'filter';
		...
		if ( $filter_mode === 'search' ) {
			$filter_by = 'search';
		} elseif ( $filter_mode === 'sorter' ) {
			$filter_by = 'sorter';
		}

		$options->put( 'filter_mode', $filter_by );
