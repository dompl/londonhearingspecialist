...
class Filter_Behavior extends Step {
	...
	public function setup_fields() {
		...
		$default_button_text = Settings::get_option( 'button_text', __( 'Apply Filters', 'woocommerce-product-filters' ) );

		// If falsey, set to default value
		if ( ! $default_button_text || $default_button_text === 'false' || $default_button_text === '0' || empty( $default_button_text ) ) {
			$default_button_text = __( 'Apply Filters', 'woocommerce-product-filters' );
		}

		$fields['wcf_options[button_text]']['value'] = $default_button_text;

		$fields['wcf_options[filter_mode]']['value'] = Settings::get_option( 'filter_mode', 'instant' );
