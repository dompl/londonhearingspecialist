...
class Attribute extends Filter implements Indexable_Interface, Storable_Interface, Filterable_Interface, Countable_Interface, Preloadable_Interface {
	...
	public function get_json_store_data() {
		...
		if ( $this->get_option( 'filter_type' ) === 'images' ) {
				...
				foreach ( $terms as $term ) {
					if ( $term instanceof \WP_Term ) {
						$terms_list[ absint( $term->term_id ) ] = [
							...
							'slug'    => $indexer->safe_value( $term->slug ),
						];
					}
				}
			}
			...
			if ( ! $indexed->isEmpty() && ! empty( $this->get_specific_terms() ) ) {
				$indexed = $indexed->filter(
					function ( $index ) {
						return in_array( $index->term_id, $this->get_specific_terms() );
					}
				);
			}
			...
			$min = floatval( $indexed->first()->facet_value );
			$max = floatval( $indexed->last()->facet_value );
			...
			if ( ! empty( $terms ) && is_array( $terms ) ) {
				foreach ( $terms as $tt ) {
					$list[] = [
						'id'    => $tt->term_id,
						'label' => $tt->name,
						'slug'  => $indexer->safe_value( $tt->slug ),
					];
				}
				$data = $list;
			}
