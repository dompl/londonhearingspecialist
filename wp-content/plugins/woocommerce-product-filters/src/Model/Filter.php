...
class Filter extends Model {
	...
	public function __construct( array $attributes = [] ) {
		...
		if ( ! empty( $taxonomies ) ) {
			foreach ( $taxonomies as $key => $label ) {
				$this->childTypes[ 'tax_' . $key ] = Taxonomy::class;
			}
		}
	...
	protected function getChildModel( array $attributes ) {
		...
		// If class name starts with "tax_", return a Taxonomy model.
		if ( Str::startsWith( $className, 'tax_' ) && ! array_key_exists( $className, $taxonomies ) ) {
			$className = Taxonomy::class;
		}
	...
	public function getInputNameAttribute() {
		$type     = $this->get_option( 'input_type' );
		$selected = Settings::get_supported_filter_types_list()[ $type ] ?? '';
