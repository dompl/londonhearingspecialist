...
class Request_Fallback {
	...
	private function attach_search_query_to_filters( Collection $collection ) {
		...
		$collection->each(
			function ( $instance ) {
				 ...
				 *
				 * This filter is mainly used internally for our own
				 * integrations, but it's also available for developers to use.
				 *
				 * @param mixed $search_query the search query to attach to the filter instance.
				 * @param Filter $instance the filter instance.
				 * @param Collection $parameters the collection of parameters.
				 * @return mixed
				 */
				$search_query_attached = apply_filters( 'wcf_fallback_attach_search_query_to_filter', $this->parameters->get( $instance->slug ), $instance, $this->parameters );

				$instance->setAttribute( 'search_query', $search_query_attached );
			}
		);
	...
	private function get_filtered_post_ids( $the_query, $additional_args = [] ) {
			...
			foreach ( $this->filters as $filter ) {
				if ( $filter instanceof Filterable_Interface ) {
					...
					if ( $use_or_logic ) {
						...
						$new_post_ids = array_merge( $new_post_ids, $matches );
		...
		if ( $has_search_input ) {
			$post_ids = array_merge( $post_ids, $cloned_posts );
