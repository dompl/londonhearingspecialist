...
class Filters {
	...
	public static function generate_taxonomy_index_data( array $defaults, string $post_id, string $taxonomy ) {
		...
		foreach ( $term_objects as $term ) {
			...
			$term_info = isset( $hierarchy[ $term->term_id ] ) ? $hierarchy[ $term->term_id ] : [ 'parent_id' => 0, 'depth' => 0 ];
	...
	public static function generate_ranged_taxonomy_index_data( array $defaults, string $post_id, string $taxonomy ) {
		$terms  = self::generate_taxonomy_index_data( $defaults, $post_id, $taxonomy );
		$output = $terms;
		...
		if ( ! empty( $output ) ) {
			foreach ( $output as $index => $term ) {
				...
				if ( isset( $output[ $index ] ) ) {
					$output[ $index ]['facet_value'] = (int) filter_var( $term['facet_display_value'], FILTER_SANITIZE_NUMBER_INT );
				}
			}
		}
		 ...
		 *
		 * @param array $output the list of terms.
		 * @param string $taxonomy the taxonomy slug.
		 * @param array $terms the original list of terms.
		 * @param string $post_id the post ID.
		 * @return array
		 */
		return apply_filters( 'wcf_taxonomy_filter_ranged_terms_list', $output, $taxonomy, $terms, $post_id );
	}
	 ...
	 * @return boolean
	 */
	public static function is_or_logic_enabled() {
		return Settings::get_option( 'or_logic', 'and' ) === 'or';
	}
	 ...
	 * @return array
	 */
	public static function get_current_page_query_params() {
		if ( get_option( 'permalink_structure' ) !== '' ) {
			return [];
		}

		// Get all the current page url query parameters.
		$query_params = Util::clean( $_GET );
		...
		foreach ( $query_params as $key => $value ) {
			...
			if ( in_array( $key, $skip_checks, true ) ) {
				continue;
			}

			if ( ! array_key_exists( $key, $registered_taxonomies ) ) {
