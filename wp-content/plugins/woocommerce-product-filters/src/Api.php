...
class Api implements Registerable {
	...
	public function get_counters( $request ) {
		...
		$is_tax            = $request->get_param( 'is_tax' );
		$taxonomy_slug     = $request->get_param( 'taxonomy_slug' );
		$queried_term_id   = $request->get_param( 'queried_term_id' );
		$queried_term_slug = $request->get_param( 'queried_term_slug' );
		...
		if ( is_array( $counts ) && $is_tax ) {
			foreach ( $counts as $slug => $query ) {
				$filter = $filters->where( 'slug', $slug )->first();

				if ( $filter instanceof Taxonomy && $filter->get_taxonomy_slug() === $taxonomy_slug ) {
					$counts[ $slug ] = Responses::cleanup_term_counters_on_taxonomy_page( $query, $queried_term_id, $taxonomy_slug, $queried_term_slug );
				}
			}
		}
	...
	public function get_terms( $request ) {
		...
		$is_attribute = $request->get_param( 'is_attribute' ) === 'true';
		...
		if ( ! $taxonomy ) {
			return new \WP_REST_Response(
				[
					'success' => false,
					'message' => __( 'No taxonomy provided.', 'woocommerce-product-filters' ),
				],
				400
			);
		}
